(function () {
    ...
    document.querySelectorAll('input[name="tipoConta"]').forEach(function (radio) {
        radio.addEventListener('change', function () {
            ...
            if (this.value === 'personagem') {
                ...
                setInvalid(document.getElementById('tipoPersonagem'), 'tipoPersonagemError', false);
                setInvalid(document.getElementById('valorHora'),      'valorHoraError',      false);
            }
        });
    });
    ...
    valorHoraEl.addEventListener('input', function () {
        ...
        this.value = 'R$ ' + reais.replace('.', ',').replace(/\B(?=(\d{3})+(?!\d))/g, '.');
    });
    ...
    senhaEl.addEventListener('input', function () {
        ...
        if (val.length === 0)      { color = 'transparent'; label = ''; }
        else if (score <= 1)       { color = '#ef4444'; label = 'Muito fraca'; }
        else if (score === 2)      { color = '#f97316'; label = 'Fraca'; }
        else if (score === 3)      { color = '#eab308'; label = 'Média'; }
        else if (score === 4)      { color = '#22c55e'; label = 'Forte'; }
        else                       { color = '#16a34a'; label = 'Muito forte'; }
    ...
    tipoPersonagemEl.addEventListener('change', function () {
        if (isPersonagemSelected()) {
            setInvalid(tipoPersonagemEl, 'tipoPersonagemError', this.value === '');
        }
    });

    valorHoraEl.addEventListener('input', function () {
        if (isPersonagemSelected()) {
            setInvalid(valorHoraEl, 'valorHoraError', parseValorHora(this.value) <= 0);
        }
    });
    ...
    document.getElementById('cadastroForm').addEventListener('submit', function (e) {
        ...
        var valorOk      = true;
        if (isPersonagemSelected()) {
            personagemOk = setInvalid(tipoPersonagemEl, 'tipoPersonagemError', tipoPersonagemEl.value === '');
